...
- **Photon iMessage**: The image ships Bun plus `packages/photon-bridge` under `/opt/spacebot` (container `WORKDIR`). If Photon is enabled and you still see `failed to spawn photon sidecar process` / `No such file or directory`, rebuild or upgrade the image—or disable Photon in **Settings → Channels** until you do. Photon on Linux containers is uncommon; confirm Spectrum supports your host before relying on it.
