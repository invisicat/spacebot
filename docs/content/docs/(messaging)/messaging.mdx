---
title: Messaging
description: How Spacebot connects to Discord, Slack, Telegram, Twitch, Email, Photon iMessage, and webhooks.
---
...
| Photon iMessage | Supported | Photon Spectrum credentials + bundled Bun sidecar |
...
- Which **platform** (Discord, Slack, Telegram, Twitch, Email, Photon, ...)
...
Named adapters use `adapter = "<name>"` in bindings. Omit `adapter` to target the platform default adapter (`discord`, `slack`, `telegram`, `twitch`, `email`, `webhook`, `mattermost`, `signal`, `photon`).
...
| Photon iMessage | Each Photon space |
...
### Photon iMessage (thread vs normal send)

The Bun `packages/photon-bridge` sidecar sends outbound text **as a threaded reply only when Photon tags the triggering inbound as referencing the bot** (quotes your agent or satisfies your mention policy → `mentions_or_replies_to_bot`). It also threads when the outgoing payload is explicitly a **thread reply** from the framework. Plain responses to untagged traffic are sent **into the space as normal messages**, not anchored as inline replies under the triggering bubble.
